...
def insert_activity(
    db_manager, timestamp, app_name, window_title, url, session_name, previous_time
):
...
def calculate_top_activities():
    ...
    sorted_activities = sorted(
        activity_duration.items(), key=lambda x: x[1], reverse=True
    )
    ...
    try:
        while True:
            ...
            if current_session:
                ...
                session_start_str = time.strftime(
                    "%H:%M %d/%m/%Y", time.localtime(session_start_time)
                )
                ...
                print(
                    f"Session started at: {session_start_str} (Elapsed: {format_elapsed_time(elapsed_time_session)})"
                )
