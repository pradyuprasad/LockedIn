...
def calculate_activity_metrics(
    activities: List[Tuple[str, str, str]], score_config: Dict[str, int]
) -> Dict[str, Any]:
    """
...
def calculate_score_components(
    metrics: Dict[str, Any], score_config: Dict[str, int]
        ...
        if duration < 30:
            if duration < 10:
                if switch_is_dev_cluster:
                    ...
                    reason = (
                        f"Short switch (<10s) in dev cluster: {from_tool} → {to_tool}"
                    )
                else:
                    penalty = score_config.get(
                ...
                if switch_is_dev_cluster:
                    ...
                    reason = (
                        f"Short switch (<30s) in dev cluster: {from_tool} → {to_tool}"
                    )
                else:
                    penalty = score_config.get(
    ...
    if task_switches > 0:
        ...
        components.append(
            ("Task Switches", switch_penalty, f"{task_switches} cluster switches")
        )
