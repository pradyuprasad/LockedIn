...
class FocusSession:
    ...
    def format_time_remaining(self) -> str:
        ...
        remaining_seconds = max(
            0, self.score_config["target_duration_minutes"] * 60 - elapsed_seconds
        )
    ...
    def check_current_activity(self) -> None:
        ...
        if violation:
            ...
            ).total_seconds() >= self.MIN_VIOLATION_TIME:
                if (
                    not self.last_violation_time
                    or (current_time - self.last_violation_time).total_seconds() > 60
                ):
    ...
    def display_session_status(self) -> None:
        ...
        print(
            f"Duration: {duration.seconds // 3600}h {(duration.seconds // 60) % 60}m {duration.seconds % 60}s"
        )
    ...
    def display_score_info(self) -> None:
        ...
        for component, value, explanation in score_data["components"]:
            if value is not None:
                ...
                color = (
                    "\033[92m" if value > 0 else "\033[91m" if value < 0 else "\033[0m"
                )
    ...
    def end(self) -> None:
        ...
        print(f"Context Switches: {len(self.violations)}")
        print(
            f"Final Deep Work Score: {final_score:.1f}/100 {get_score_emoji(final_score)}"
        )
        ...
        if self.violations:
            ...
            for v in self.violations:
                violation_counts[v["app_or_site"]] = (
                    violation_counts.get(v["app_or_site"], 0) + 1
                )

            for app_or_site, count in sorted(
