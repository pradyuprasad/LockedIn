...
class SessionManager:
    ...
    def get_session_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "formatted_elapsed_time_overall": format_elapsed_time(int(time.time() - self.overall_start_time))
        }
        
        if self.session_start_time:
            elapsed_time_session = int(time.time() - self.session_start_time)
            info["elapsed_time_session"] = elapsed_time_session
            info["session_start_str"] = time.strftime("%H:%M %d/%m/%Y", time.localtime(self.session_start_time))
            info["formatted_elapsed_time_session"] = format_elapsed_time(elapsed_time_session)
